...
function ChecklistCard({ title, items }: { title: string; items: ChecklistItem[] }) {
  const open = items.filter(i => !i.done).length;
  return (
    <section className="rounded-2xl border border-warm-200 bg-white p-5 shadow-soft">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <h2 className="font-serif text-lg font-semibold text-accent">{title}</h2>
      ...
      {items.length === 0 ? (
        ...
        <ul className="mt-4 space-y-2" role="list">
          {items.map((item, idx) => (
            <li
              key={`${title}-${idx}-${item.text.slice(0, 40)}`}
    ...
    <main className="min-h-screen bg-warm-50 text-accent">
      <div className="border-b border-warm-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-4xl flex-col gap-2 px-4 py-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="font-serif text-2xl font-semibold text-accent sm:text-3xl">待办与开发事项</h1>
      ...
      <div className="mx-auto max-w-4xl space-y-10 px-4 py-10">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <ChecklistCard title="待办" items={board.todos} />
        ...
        <footer className="border-t border-warm-200 pb-8 pt-6 text-center text-xs text-gray-500">
          会后可将记录交给 AI，按本文档章节结构输出 Markdown，再粘贴进仓库并部署。
        </footer>
      </div>
    </main>
  );
