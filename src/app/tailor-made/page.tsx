...
const ideas = [
  {title:'Your first China journey',body:'Great landmarks, a comfortable pace, and room for discovery.',image:'planner-palace',alt:'Palace architecture in Beijing'},
  {title:'Slow down and explore',body:'Lakeside walks, gardens and time to experience everyday life.',image:'planner-westlake',alt:'West Lake in Hangzhou'},
  {title:'Follow the flavours',body:'Regional food, local markets and the places behind each dish.',image:'planner-food',alt:'Chinese dishes shared around a table'},
];
const faqs = [
...
export default function TailorMadePage() {
 return <div className="bg-surface text-ink">
  ...
  <section className="mx-auto max-w-7xl px-4 py-16"><p className="text-xs font-semibold uppercase tracking-widest text-primary">A little inspiration</p><h2 className="mt-3 font-serif text-3xl">There’s more than one way to see China.</h2><p className="mt-4 text-ink-muted">A starting point, not a fixed itinerary. Choose an idea to include in your enquiry.</p><div className="mt-8 grid gap-6 md:grid-cols-3">{ideas.map(i=><Link key={i.title} href={'/tailor-made?idea='+encodeURIComponent(i.title)+'#enquiry-form'} className="overflow-hidden rounded-2xl border border-warm-200 bg-white"><div className="relative aspect-[16/10]"><Image unoptimized src={'/images/cts-upgrade/'+i.image+'.webp'} alt={i.alt} fill sizes="(max-width:768px) 100vw, 33vw" className="object-cover"/></div><div className="p-6"><h3 className="font-serif text-xl">{i.title}</h3><p className="mt-3 text-sm leading-relaxed text-ink-muted">{i.body}</p><p className="mt-5 font-semibold text-primary">Make it mine →</p></div></Link>)}</div></section>
