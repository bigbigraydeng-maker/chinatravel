...
export default function GoldenTourPage({tour}:{tour:Tour}) {
 const related=getAllActiveTours().filter(t=>t.slug!==tour.slug && t.destination===tour.destination && matchingOffer(t,{})).sort((a,b)=>Number(b.departureDates?.some(d=>d.includes('December 2026')))-Number(a.departureDates?.some(d=>d.includes('December 2026')))).slice(0,3);
 return <div className="bg-surface text-ink">
  <div className="mx-auto max-w-7xl px-4 pb-10 pt-6"><nav aria-label="Breadcrumb" className="mb-8 text-sm text-ink-muted"><Link href="/">Home</Link> / <Link href="/tours/find">Tours</Link> / {tour.name}</nav>
   ...
   {tour.soldOut && <section className="mb-8 rounded-2xl border border-primary/20 bg-white p-6"><h2 className="font-serif text-3xl text-primary">16 November 2026 · Sold Out</h2><p className="mt-3 text-ink-muted">This Golden China departure is fully booked. Join our Christmas &amp; New Year journeys departing Auckland or Christchurch on 22 December 2026.</p><Link href="/campaigns/spotlight" className="mt-4 inline-block font-semibold text-primary">Explore December tours →</Link></section>}
   <JourneyGallery images={photos}/>
  </div><TrustBar/><TourTrustSignals/>
  <nav aria-label="On this page" className="border-y border-warm-200 bg-white"><div className="mx-auto flex max-w-7xl flex-wrap gap-x-7 gap-y-3 px-4 py-5 text-sm font-semibold">{[['overview','Overview'],['route-map','Route map'],['itinerary','Day by day'],['inclusions','Inclusions'],['planning-resources','Travel guides'],['enquiry',tour.soldOut ? 'December alternatives' : 'Enquire']].map(([id,label])=><a key={id} href={'#'+id} className="text-primary hover:underline">{label}</a>)}</div></nav>
